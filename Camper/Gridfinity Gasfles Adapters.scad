...
hole_diameter = 32; // Metric size (e.g., M8)
head_height = height * 7;
tolerance = 0.5; // Adds a small gap for fitment
...
difference() {
    difference () {
        ...
        for ( column  =  [0 : width - 1] ){
            // The bolt hole
            union() {
                translate([(column * 41.5),0,5]) {
                    cylinder(d = hole_diameter + tolerance, h = head_height, $fn = 6);
                };
    ...
    for ( column  =  [0 : width - 1] ){
        translate([(column * 41.5),0,grabDiameter/1.3]){
            union() {
                sphere(d=grabDiameter);
                cylinder(h=(grabDiameter*2), d=grabDiameter);
            };
        }
    }
}